...
def build_cmd(out_dir, cookie, sleep, url, mode="photos"):
    ...
    if mode == "photos":
        ...
        cmd += ["--chapter-filter", "type == 'story'"]
    elif mode == "highlights":
        cmd += ["--chapter-filter", "type == 'highlight'"]
    if cookie.exists():
...
# ── SEND MEDIA ────────────────────────────────────────────────────────────────
async def send_as_photos(target, batch: list[Path]):
    """Send batch strictly as photos."""
    if not batch: return
    try:
...
async def send_as_videos(target, batch: list[Path]):
    """Send batch strictly as videos."""
    if not batch: return
    try:
...
async def send_as_documents(target, batch: list[Path]):
    """Send batch as documents."""
    for f in batch:
        try:
...
async def realtime_download(send_target, out_dir: Path, cookie: Path,
    ...
    if mode in ("photos", "stories", "highlights"):
        exts = PHOTO_EXT | VIDEO_EXT  # stories/highlights can be either
        send_fn_photo = send_as_photos
        send_fn_video = send_as_videos
    elif mode == "videos":
        exts = VIDEO_EXT
        send_fn_photo = None
        send_fn_video = send_as_videos
    ...
    async def flush_all():
        ...
        if photo_buf:
            if mode == "documents":
                await send_as_documents(send_target, list(photo_buf))
            else:
                await send_as_photos(send_target, list(photo_buf))
            sent += len(photo_buf); photo_buf.clear()
        if video_buf:
            if mode == "documents":
                await send_as_documents(send_target, list(video_buf))
            else:
                await send_as_videos(send_target, list(video_buf))
            sent += len(video_buf); video_buf.clear()
        if doc_buf:
            await send_as_documents(send_target, list(doc_buf))
            sent += len(doc_buf); doc_buf.clear()
    ...
    while proc.returncode is None:
        ...
        if out_dir.exists():
            for f in sorted(out_dir.iterdir()):
                ...
                if mode == "documents":
                    doc_buf.append(f)
                    if len(doc_buf) == 10: await flush_all()
                elif kind == "photo" and mode in ("photos", "stories", "highlights", "both"):
                    photo_buf.append(f)
                    if len(photo_buf) == 10: await flush_all()
                elif kind == "video" and mode in ("videos", "stories", "highlights", "both"):
                    video_buf.append(f)
                    if len(video_buf) == 10: await flush_all()
        try:
