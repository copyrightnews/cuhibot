...
domain = os.environ.get("PUBLIC_DOMAIN") or os.environ.get(
...
def start_mini_app_server():
    if os.environ.get("SKIP_EMBEDDED_SERVER") == "1":
        logger.info(
            "SKIP_EMBEDDED_SERVER is set to 1 — skipping embedded FastAPI server thread"
        )
        return
    try:
        port = int(os.environ.get("PORT", 8080))
        t = threading.Thread(
            target=_server_module.start, args=(port,), daemon=True
        )
        t.start()
        if MINI_APP_URL:
            logger.info(
                "Mini App server started → %s (port %d)", MINI_APP_URL, port
            )
        else:
            logger.warning(
                "PUBLIC_DOMAIN not set — Mini App server started on port %d but URL is empty",
                port,
            )
...
async def atomic_edit_profiles(
    uid: int, platform: str, func: callable
) -> None:
    await asyncio.get_running_loop().run_in_executor(
...
def kb_main() -> InlineKeyboardMarkup:
    rows = [
        ...
        [
            ...
            InlineKeyboardButton(
                "✨ Highlights", callback_data="m_highlights"
            ),
        ],
        [
        ...
        [
            InlineKeyboardButton(
                "📎 Export sources", callback_data="m_export"
            ),
...
def kb_platforms(prefix: str) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(p.capitalize(), callback_data=f"{prefix}_{p}")]
        for p in PLATFORMS
    ]
...
def kb_media() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "🔖 Both (separately)", callback_data="dl_3"
                )
            ],
...
async def send_menu(
    msg, uid: int, username: str, name: str, *, edit=False
) -> None:
...
async def _send_one(
    ...
    *,
    ...
    if fsize > TELEGRAM_FILE_LIMIT:
        logger.warning(
            "Skipping %s (%.1f MB) — exceeds limit",
            f.name,
            fsize / (1024 * 1024),
        )
        return False
...
async def flush(
    target, batch: list[Path], send_as: str, stop: asyncio.Event | None = None
) -> int:
                ...
                for _retries in range(5):
                    ...
                    with ExitStack() as stack:
                        try:
                            ...
                            logger.warning(
                                "Group send failed: %s, falling back", str(e)
                            )
                            break
    ...
    try:
        while proc.returncode is None:
            ...
            if await asyncio.to_thread(out_dir.exists):
                ...
                if candidates:
                    ...
                    for f in candidates:
                        try:
                            initial_sizes[f] = await asyncio.to_thread(
        ...
        if status:
            if (
                ...
                await status.set(
                    f"⚠️ `{handle}` → Failed or blocked. (Exit: {proc.returncode})",
                    force=True,
                )
            else:
                await status.set(
...
async def do_download(
    msg, choice: str, uid: int, uname: str, name: str, bot, stop: asyncio.Event
) -> None:
...
def _prune_rate_limits() -> None:
    now = time.time()
    stale = [
        k for k, v in _LAST_DOWNLOAD.items() if (now - v) > RATE_LIMIT_SECONDS
    ]
    for k in stale:
...
async def cmd_cleanup(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in ACTIVE_USERS:
        await update.message.reply_text(
            "⚠️ Please stop the active download first."
        )
        return
...
async def handle_callback(
    ...
    if data.startswith("rem_"):
        ...
        if not urls:
            await q.message.edit_text(
                f"🚫 No sources for *{platform.capitalize()}*.",
                parse_mode="Markdown",
                reply_markup=kb_back(),
            )
            return
    ...
    if data == "m_list":
        ...
        await q.message.edit_text(
            text[:3900], parse_mode="Markdown", reply_markup=kb_back()
        )
        return
    ...
    if data.startswith("dl_"):
        ...
        _record_download_time(uid)
        start_download_task(
            uid, do_download, q.message, choice, uid, uname, name, ctx.bot
        )
        return
    ...
    if data == "m_highlights":
        rows = [
            [InlineKeyboardButton("Instagram", callback_data="hl_instagram")],
            [InlineKeyboardButton("🔙 Back", callback_data="m_back")],
        ]
        await q.message.edit_text(
    ...
    if data == "m_history":
        ...
        lines = [
            f"📅 `{e.get('date')}` | *{e.get('platform')}* › `{e.get('user')}` | {e.get('sent')} file(s)"
            for e in entries[:20]
        ]
        await q.message.edit_text(
            "\n".join(lines)[:3900] if lines else "🚫 No history.",
            parse_mode="Markdown",
            reply_markup=kb_back(),
        )
        return
    ...
    if data == "m_status":
        ...
        await q.message.edit_text(
            text, parse_mode="Markdown", reply_markup=kb_back()
        )
        return
    ...
    if data == "m_export":
        ...
        if not lines:
            await q.message.edit_text(
                "🚫 No sources to export.", reply_markup=kb_back()
            )
            return
        ...
        with open(export_file, "rb") as fh:
            await q.message.reply_document(
                document=fh, filename="cuhibot_sources.txt"
            )
...
async def cmd_stories(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not _is_allowed(uid):
        return
    rows = [
        [InlineKeyboardButton("Instagram", callback_data="story_instagram")],
        [InlineKeyboardButton("🔙 Back", callback_data="m_back")],
    ]
    await update.message.reply_text(
        "📖 *Stories* — pick a platform:",
...
async def cmd_link(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    uid, uname, name = _user(update)
    if not _is_allowed(uid) or not ctx.args:
        return
...
async def _scheduled_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in ACTIVE_USERS:
        return
    if not any(await read_profiles(uid, p) for p in PLATFORMS):
        return
    ev = asyncio.Event()
    STOP_EVENTS[uid] = ev
    ACTIVE_USERS.add(uid)
...
async def sync_user_schedule(app: Application, uid: int) -> None:
    ...
    try:
        ...
        active_jobs = (
            app.job_queue.get_jobs_by_name(job_name) if app.job_queue else []
        )
...
async def _restore_schedules(app: Application) -> None:
    if not await asyncio.to_thread(DATA_ROOT.exists):
        return
...
async def _run_miniapp_download(
    ...
    try:
        ...
        try:
            await bot.send_message(
                chat_id=uid, text=f"❌ Mini App download error: {exc}"
            )
        except Exception:
            pass
    finally:
        ...
        try:
            ...
            if q_type == "stop":
                ...
                t = asyncio.ensure_future(
                    _run_miniapp_download(uid, trigger_data, bot)
                )
                _TASKS.add(t)
    ...
    while True:
        try:
            if MINIAPP_QUEUE is not None and DATA_ROOT.exists():
                ...
                for stop_file in DATA_ROOT.glob("*/stop_flag"):
                    try:
                        ...
                        if uid in ACTIVE_USERS and uid in STOP_EVENTS:
                            STOP_EVENTS[uid].set()
                            logger.info(
                                "Local loop STOP triggered for uid=%s", uid
                            )
                    except Exception:
                        continue
                ...
                if tick % 10 == 0:
                    for user_dir in await asyncio.to_thread(
                        lambda: list(DATA_ROOT.iterdir())
                    ):
                        if (
                            not user_dir.is_dir()
                            or not user_dir.name.isdigit()
                        ):
                            continue
                        uid = int(user_dir.name)
                        await sync_user_schedule(app, uid)
...
def main() -> None:
    ...
    if DATA_ROOT.exists():
        for run_flag in DATA_ROOT.glob("*/download_running"):
            try:
                ...
                logger.info(
                    "Cleared stale download_running flag on startup: %s",
                    run_flag,
                )
            except Exception:
                pass
    ...
    if app.job_queue is not None:
        ...
        app.job_queue.run_repeating(
            poll_miniapp_queue_fallback,
            interval=30,
            first=10,
            name="miniapp_fallback",
        )
